                        ...
                        <form id="productForm">
                            <div class="border p-4 rounded">
                                ...
                                <div class="row mb-3">
                                    ...
                                    @if ($data->image != '')
                                        <div class="col-sm-9">
                                            <img style="width: 100px" src="{{ asset($data->image) }}" alt=""
                                                width="100px">
                                        </div>
                                    @endif
                                </div>
                                        ...
                                        <select name="category" onchange="getAttributes()" class="form-control">
                                            @foreach ($category as $list)
                                                @if ($data->category_id == $list->id)
                                                    <option selected value="{{ $list->id }}">{{ $list->name }}
                                                    </option>
                                                @else
                                            ...
                                            @if (isset($data->attributes[0]->id))
                                                @foreach ($attributes as $attr)
                                                    @foreach ($attr->values as $value)
                                                        <?php
                                                        $isSelected = '';
                                                        foreach ($data->attributes as $sAttr) {
                                                            if ($sAttr->attribute_values->id == $value->id) {
                                                                $isSelected = 'selected';
                                                            }
                                                        }
                                                        ?>
                                                        <option {{ $isSelected }} value="{{ $value->id }}">
                                                            {{ $attr->attribute->name }}
                                                            ({{ $value->value }})
                                                        </option>
                                                    @endforeach
                                            ...
                                            <div class="row" id='attrRec_{{ $count }}'>
                                                <div class="col-sm-3">
                                                    <select name="color[]" id="color" class="form-control">
                                                        @foreach ($color as $list)
                                                            <option <?php echo $pAttr->color_id == $list->id ? 'selected' : ''; ?> value="{{ $list->id }}">
                                                                {{ $list->text }} </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-sm-3">
                                                    <select name="size[]" id="size" class="form-control">
                                                        @foreach ($size as $list)
                                                            <option <?php echo $pAttr->size_id == $list->id ? 'selected' : ''; ?> value="{{ $list->id }}">
                                                                {{ $list->text }} </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-sm-3">
                                                    <input type="text" class="form-control" id="sku"
                                                        name="sku[]" rows="3" value="{{ $pAttr->sku }}"
                                                        placeholder="Enter SKU">
                                                </div>
                                                <div class="col-sm-3">
                                                    <input type="text" class="form-control" id="mrp"
                                                        name="mrp[]" rows="3" value="{{ $pAttr->mrp }}"
                                                        placeholder="Enter MRP">
                                                </div>
                                                <div class="col-sm-3">
                                                    <input type="text" class="form-control" id="price"
                                                        name="price[]" rows="3" value="{{ $pAttr->price }}"
                                                        placeholder="Enter Price">
                                                </div>
                                                <div class="col-sm-3">
                                                    <input type="text" class="form-control" id="length"
                                                        name="length[]" value="{{ $pAttr->length }}" rows="3"
                                                        placeholder="Enter length">
                                                </div>
                                                <div class="col-sm-3">
                                                    <input type="text" class="form-control" id="breadth"
                                                        name="breadth[]" value="{{ $pAttr->breadth }}" rows="3"
                                                        placeholder="Enter Breadth">
                                                </div>
                                                <div class="col-sm-3">
                                                    <input type="text" class="form-control" id="height"
                                                        name="height[]" rows="3" value="{{ $pAttr->height }}"
                                                        placeholder="Enter Height">
                                                </div>
                                                <div class="col-sm-3">
                                                    <input type="text" class="form-control" id="weight"
                                                        name="weight[]" value="{{ $pAttr->weight }}" rows="3"
                                                ...
                                                <div class="row mb-3">
                                                    ...
                                                    <div class=" row col-sm-9">
                                                        <input type="hidden" name="image_values[]"
                                                            value="{{ $count }}" />
                                                        <div class="col-sm-3">
                                                            <button type="button"
                                                                onclick="addAttrImage({{ $count }})"
                                                                id='addAttrImages' class="btn btn-info">Add
                                                                images</button>
                                                        </div>
                                                        <div class="row" id='attrImages_{{ $count }}'>

                                                            @foreach ($pAttr->images as $paImage)
                                                                <div class="col-sm-3">
                                                                    <img src="{{ asset($paImage->image) }}"
                                                                        alt="" width="100px">
                                                                    <button type="button"
                                                                        onclick="removeAttrImage(event);deleteData('{{ $paImage->id }}','product_attr_images')"
                                                                        id='addAttrImages' class="btn btn-danger">Remove
                                                                        {{ $paImage->id }}</button>
                                                                </div>
                                                            @endforeach
        ...
        $(document).ready(function() {
            $('#addAttributeButton').click(function() {
                $('#addAttr').append('  <div class="row" id="attrRec_' + attrCounter +
                    ...
                    '                    <div class=" row col-sm-9"><input type="hidden" name="image_values[]" value="' +
                    attrCounter + '"/>' +
                    '                        <div class="col-sm-3">' +
